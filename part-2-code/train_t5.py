...
from load_data import load_t5_data, tokenizer as data_tokenizer
...
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(train_loader):
        optimizer.zero_grad()
        encoder_input = encoder_input.to(DEVICE).contiguous()
        encoder_mask  = encoder_mask.to(DEVICE).contiguous().long()
        #decoder_input = decoder_input.to(DEVICE) 
        decoder_targets = decoder_targets.to(DEVICE).contiguous()
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(dev_loader):
            ...
            loss = outputs.loss
            num_tok = (decoder_targets != PAD_IDX).sum().item()
            ...
            pred_sql = tokenizer.batch_decode(gen_idx, skip_special_tokens=True)
            pred_sql = [s.strip() for s in pred_sql]
            pred_sql_list.extend(pred_sql)
        
     
        save_queries_and_records(pred_sql_list, model_sql_path, model_record_path)
