...
def create_augmented_dataloader(args, dataset):
    ...
    ori_train = dataset['train']
    augmented_subset = ori_train.shuffle(seed=42).select(range(5000))
    transformed_subset = augmented_subset.map(custom_transform, load_from_cache_file=False)
    
    assert "text" in transformed_subset.column_names, transformed_subset.column_names
    assert "label" in transformed_subset.column_names, transformed_subset.column_names
    
    augmented_train = concatenate_datasets([ori_train, transformed_subset])
    #print(f"Combined dataset: {len(ori_train)} original + {len(transformed_subset)} transformed")
    ...
    assert tokenized_train is not None, "⚠️ tokenized_train is None — check tokenize_function!"
    ...
    train_dataloader = DataLoader(
        tokenized_train, shuffle=True, batch_size=args.batch_size)
    #print(f"Augmented DataLoader ready: {len(tokenized_train)} examples")
